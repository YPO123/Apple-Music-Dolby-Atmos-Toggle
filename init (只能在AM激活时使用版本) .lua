...
-- 仅在 Apple Music 前台时启用的热键（默认禁用）
local hk_atmos  = hs.hotkey.new({"cmd"}, "a", atmos_force_auto) -- ⌘A → Atmos
local hk_stereo = hs.hotkey.new({"cmd"}, "s", atmos_force_off)  -- ⌘S → Stereo
hk_atmos:disable(); hk_stereo:disable()

-- 状态位，避免重复 enable/disable
local hotkeysEnabled = false

local function isMusicFrontmost()
  local ok, front = pcall(hs.application.frontmostApplication)
  if not ok or not front then return false end
  return front:bundleID() == MUSIC_BUNDLE
end

local function setHotkeys(on)
  if on and not hotkeysEnabled then
    hk_atmos:enable(); hk_stereo:enable()
    hotkeysEnabled = true
  elseif (not on) and hotkeysEnabled then
    hk_atmos:disable(); hk_stereo:disable()
...
local function syncHotkeys()
  setHotkeys(isMusicFrontmost())
end

-- 用系统默认的 window.filter 监听前台切换（更靠谱）
...
}, function() syncHotkeys() end)

-- 辅助：app watcher 也同步一次（处理无窗口或菜单栏激活等边界）
local appWatcher = hs.application.watcher.new(function(_, eventType, _)
  if eventType == hs.application.watcher.activated
     ...
     or eventType == hs.application.watcher.unhidden
     or eventType == hs.application.watcher.hidden then
    syncHotkeys()
  end
end):start()

-- 兜底定时器：每 1.5 秒校正一次（轻量）
local guardTimer = hs.timer.doEvery(1.5, syncHotkeys)
